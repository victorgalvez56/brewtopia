...
const List: React.FC<Props> = observer(({ list, onDetails, onPressFavorite, type }) => {
  return (
    <FlatList
      data={list}
      ...
      renderItem={({ item }) => (
        <View style={tw`flex-row items-start justify-between`}>
          <Pressable onPress={() => onDetails(item)}>
            <ImageBackground
          ...
          <View style={tw`flex-1 ml-3`}>
            <TouchableOpacity onPress={() => onDetails(item)}>
              ...
              <View style={tw`flex-row items-center justify-between`}>
                <Text style={tw`text-sm text-brewtopiagray-700`}>{/* {item.abv || 0}% ABV */}</Text>
